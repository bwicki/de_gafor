...
const DWD = (() => {
  ...
  /** Der ausführliche Bericht liegt je Gebiet in einer eigenen Datei. */
  const balloonCache = new Map();
  async function loadBalloon(id) {
    const key = String(id);
    if (balloonCache.has(key)) return balloonCache.get(key);
    const entry = balloonFor({ id: key });
    if (!entry) return null;
    const p = U.getJSON(entry.file ? `${entry.file}?t=${Math.floor(Date.now() / 600000)}`
                                   : `data/dwd/balloon/${key}.json`)
      .catch(e => { console.warn('Ballonbericht', key, e.message); return null; });
    balloonCache.set(key, p);
    return p;
  }
