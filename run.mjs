...
head('Symbole');
{
  ...
  const need = ['favicon.svg', 'favicon.ico', 'icon-192.png', 'icon-512.png',
                'icon-maskable-512.png', 'apple-touch-icon.png'];
  const miss = [];
  for (const f of need) { try { await stat(`icons/${f}`); } catch { miss.push(f); } }
  miss.length ? bad(`fehlende Symbole: ${miss.join(', ')}`)
              : ok(`${need.length} Symboldateien vorhanden`);

  const svg = await readFile('icons/favicon.svg', 'utf8');
  /viewBox="0 0 100 100"/.test(svg) ? ok('favicon.svg hat das erwartete Koordinatenfeld')
                                    : bad('favicon.svg ohne viewBox 0 0 100 100');
  // keine Schrift im Symbol: die fehlt auf fremden Geräten
  !/<text|font-family/.test(svg) ? ok('keine Schrift im Symbol')
                                 : bad('favicon.svg enthält Text oder eine Schriftangabe');
  // die vier Stufenfarben des hellen Farbsatzes
  const cols = [...svg.matchAll(/fill="(#[0-9a-f]{6})"/gi)].map(m => m[1].toLowerCase());
  ['#1d8a56', '#6d9418', '#b96f14', '#d1631a'].every(c => cols.includes(c))
    ? ok('die vier GAFOR-Stufenfarben stecken drin') : bad(`Farben: ${cols.join(' ')}`);
  cols.includes('#f2f4f7') ? ok('heller Grund') : bad('der Grund ist nicht hell');

  /* Alles muss innerhalb des Kreises liegen, den Android aus einem maskablen
     Symbol schneidet (Radius 40 um die Mitte) — sonst fehlen die Randbalken. */
  const rects = [...svg.matchAll(/<rect x="([\d.]+)" y="([\d.]+)" width="([\d.]+)" height="([\d.]+)"[^>]*fill="#[0-9a-f]{6}"/gi)]
    .map(m => m.slice(1, 5).map(Number))
    .filter(r => r[2] < 90);                      // die Grundfläche ausnehmen
  const worst = Math.max(...rects.flatMap(([x, y, w, h]) =>
    [[x, y], [x + w, y], [x, y + h], [x + w, y + h]]
      .map(([px, py]) => Math.hypot(px - 50, py - 50))));
  rects.length === 4 && worst <= 40.5
    ? ok(`der äusserste Punkt liegt ${worst.toFixed(1)} von der Mitte — im maskablen Kreis`)
    : bad(`${rects.length} Balken, äusserster Punkt bei ${worst.toFixed(1)} (erlaubt 40)`);

  // die PNG tragen ihre Kantenlänge im Namen und im Kopf
  const png = async (f) => {
    const b = await readFile(`icons/${f}`);
  ...
  for (const [f, n] of Object.entries(sizes)) {
    const [w, h] = await png(f);
    if (w !== n || h !== n) wrong.push(`${f}: ${w}×${h}`);
  }
  wrong.length ? bad(`falsche Kantenlängen: ${wrong.join(', ')}`)
               : ok('alle PNG haben die Kantenlänge, die ihr Name verspricht');

  // das maskable Symbol darf keine durchsichtigen Ecken haben
  {
    const b = await readFile('icons/icon-maskable-512.png');
    b.readUInt8(25) === 6 || b.readUInt8(25) === 2
      ? ok(`maskables Symbol im Farbtyp ${b.readUInt8(25)} (randlos gefüllt)`)
      : bad(`unerwarteter PNG-Farbtyp: ${b.readUInt8(25)}`);
  }

  // index.html und manifest müssen dieselben Dateien nennen
  const html = await readFile('index.html', 'utf8');
  const man = JSON.parse(await readFile('manifest.webmanifest', 'utf8'));
  ['favicon.svg', 'favicon.ico', 'apple-touch-icon.png'].every(f => html.includes(`icons/${f}`))
    ? ok('index.html verweist auf SVG, ICO und Apple-Symbol') : bad('ein Verweis fehlt in index.html');
  man.icons.length === 3 && man.icons.some(i => i.purpose === 'maskable')
    ? ok('das Manifest führt drei Symbole samt maskabler Fassung')
